...
public enum ReviewStatusReason {
    ...
    RESOLVED_BLIND_REASON_03(BLINDED, "내용과 맞지 않는 평점을 남긴 경우", false),
    RESOLVED_BLIND_REASON_04(BLINDED, "평점 조작 어뷰징 리뷰", false),
    ...
    RESOLVED_DELETED_REASON_04(DELETED, "리모델링, 사업주 변경 (포인트 차감 없음)", false),
    ...
    RESOLVED_DELETED_REASON_08(DELETED, "숙소를 이용하지 않고 작성된 게시물 (포인트 차감 없음)", false)
    ;
     ...
     * 리뷰상태별 사유 목록
     */
    public static List<ReviewStatusReason> getListBy(ReviewStatus... reviewStatus) {
        List<ReviewStatus> searchParams = Arrays.stream(reviewStatus).toList();
        return Arrays.stream(values())
            .filter(o -> searchParams.contains(o.getReviewStatus()))
            .toList();
    }
